...
export function ListingCard({ listing }: ListingCardProps) {
  ...
  return (
    <Link href={`/listings/${listing.slug}`} className="group block rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      {/* Photo */}
      <div className="relative h-40 w-full bg-gray-100">
        {photo ? (
          <Image src={photo.url} alt={photo.caption ?? listing.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width: 640px) 100vw, 320px" />
        ) : (
        ...
        {listing.isVerified && (
          <div className="absolute top-2 right-2">
            <Badge variant="green" className="gap-1">✓ Verified</Badge>
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <p className="mt-0.5 text-sm text-gray-500">{listing.city}, {listing.state}</p>
        ...
        <div className="mt-2 flex items-center gap-3 text-sm">
          ...
          {listing.distance !== null && (
            <span className="text-gray-400">{listing.distance.toFixed(1)} mi</span>
          )}
          {listing.priceTier && (
            <span className="text-gray-500">{PRICE_TIER_LABELS[listing.priceTier]}</span>
          )}
        </div>

        {listing.ageMin !== null && (
          <p className="mt-1 text-xs text-gray-400">Ages {listing.ageMin}–{listing.ageMax ?? 18}</p>
        )}

        {listing.programTypes.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {listing.programTypes.slice(0, 3).map((p) => (
              <span key={p} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-600 capitalize">{p}</span>
            ))}
          </div>
        )}
      </div>
